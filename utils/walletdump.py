...
from wallet.monero.backends.jsonrpc import JSONRPCWallet, RPCError
...
def pmt2str(pmt):
    res = [
        "{time} {height:7d} {hash} {amount:17.12f} {fee:13.12f} {addr} {payment_id}".format(
            time=pmt.timestamp.strftime("%d-%m-%y %H:%M:%S")
            if getattr(pmt, "timestamp", None)
            else None,

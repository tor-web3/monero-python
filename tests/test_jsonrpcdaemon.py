...
class JSONRPCDaemonTestCase(JSONTestCase):
    ...
    def test_block(self):
        ...
        self.assertEqual(
            blk.hash, "423cd4d170c53729cf25b4243ea576d1e901d86e26c06d6a7f79815f3fcb9a89"
        )
        self.assertEqual(blk.height, 451992)
        self.assertIsInstance(blk.reward, decimal.Decimal)

        self.assertIn(
            "24fb42f9f324082658524b29b4cf946a9f5fcfa82194070e2f17c1875e15d5d0", blk
        )
        ...
        # tx not in block
        self.assertNotIn(
            "e3a3b8361777c8f4f1fd423b86655b5c775de0230b44aa5b82f506135a96c53a", blk
        )
        # wrong arg type
    ...
    def test_get_block_template(self):
        ...
        self.assertEqual(
            resp["prev_hash"],
            "fbe94dbaf09e5fc4431460ede46c0e7c3db4adf1f0a6f34c5a0f5144e6d9dd27",
        )
        ...
        with self.assertRaises(ValueError):
            self.backend.get_block_template(
                "ThisIsAnInvalidWalletAddressSoThisShouldThrowAnError", 30
            )
    ...
    def test_get_last_block_header(self):
        ...
        self.assertEqual(
            block_header["wide_cumulative_difficulty"], "0x1330ce5bf1eabc9"
        )
        ...
        self.assertEqual(
            block_header["hash"],
            "a55ec867052340715c4b8b4dcd2de53bc2a195e666058d10a224037932ccdc40",
        )
        ...
        self.assertEqual(
            block_header["miner_tx_hash"],
            "42219818a7f30910a89e0d0d9fc479950137b93820e5955fc071fa8f4e3c2400",
        )
        ...
        self.assertEqual(
            ...
            "7ca630666d7040f0cadbaaf9da92db4797ef67b60ca8f15324b94236ffe0b3a8",
        )
    ...
    def test_get_block_header_by_hash(self):
        ...
        resp = self.backend.get_block_header_by_hash(
            "90f6fd3fe29c7518f15afd2da31734e890cc24247b5da10dc9ac2ea215ae844b"
        )
        ...
        self.assertEqual(block_header["cumulative_difficulty_top64"], 0)
        self.assertEqual(
            block_header["wide_cumulative_difficulty"], "0x133cfb3858fc7fd"
        )
        ...
        self.assertEqual(
            block_header["miner_tx_hash"],
            "5e8d9531ae078ef5630e3c9950eb768b87b31481652c2b8dafca25d57e9c0c3f",
        )
        ...
        self.assertEqual(
            block_header["prev_hash"],
            "a78d9e631f743806b0b8d3a70bb85758db466633eb3b4620737dd29b0548eb21",
        )
        self.assertEqual(block_header["reward"], 1176972120146)
        self.assertEqual(block_header["timestamp"], 1612323209)

        with self.assertRaises(ValueError):
            self.backend.get_block_header_by_hash(
                "HeyWaitAMinuteThisIsntAHashYouLiedToMeHowCouldYouDoThisToMeITrustedYou"
            )
    ...
    def test_get_block_header_by_height(self):
        ...
        self.assertEqual(block_header["cumulative_difficulty_top64"], 0)
        self.assertEqual(
            block_header["wide_cumulative_difficulty"], "0x133d8c662b9d436"
        )
        ...
        self.assertEqual(
            block_header["hash"],
            "966c1a70358ce998e7d5fb243b155971f9bffe06030c92dbd70486d398c40c05",
        )
        ...
        self.assertEqual(
            block_header["miner_tx_hash"],
            "408dd52531cab37e51db5a9a581bf25691b5534d8d0037b38e68061691b976e1",
        )
        ...
        self.assertEqual(
            block_header["prev_hash"],
            "47751e6eb31230e92a5ee98242aa34d79bfd48657f2727c9a9b3cbad6aee88bb",
        )
    ...
    def test_get_block_headers_range(self):
        ...
        self.assertEqual(
            block_header_0["hash"],
            "01a5a129515e752055af9883ac98cdbd9eb90db16ab69ea187a1eb24eb7d0c66",
        )
        ...
        self.assertEqual(
            block_header_0["miner_tx_hash"],
            "22c3dd706931fe0606e086147dcb8a984b504a5bd0eabd1cf7dabb9456154cd4",
        )
        ...
        self.assertEqual(
            block_header_0["prev_hash"],
            "6bd071b487be697128142b9b132be8d2c3e4ee9660f73d811c3d23e4526e56ac",
        )
    ...
    def test_get_block_with_height(self):
        ...
        self.assertTrue(
            resp["blob"].startswith("0e0efef0e880066d78ace422007a2cefb423553b3")
        )
    ...
    def test_get_bans(self):
        ...
        self.assertIn(
            "bans", resp
        )  # Should always be true bc I insert bans field when not given
        bans = resp["bans"]
        ban0 = bans[0]
    ...
    def test_flush_txpool_single(self):
        ...
        self.backend.flush_txpool(
            "65971f85ee13782f1f664cc8034a10b361b8b71ef821b323405ee0f698adb702"
        )
    ...
    def test_flush_txpool_multiple(self):
        ...
        txs_to_flush = [
            "51bd0eebbb32392d4b4646e8b398432b9f42dee0e41f4939305e13e9f9a28e08",
    ...
    def test_get_alternate_chains(self):
        ...
        self.assertEqual(
            chain0["block_hash"],
            "697cf03c89a9b118f7bdf11b1b3a6a028d7b3617d2d0ed91322c5709acf75625",
        )
    ...
    def test_relay_tx_multiple(self):
        ...
        txs = [
            "70ee3df366870d8ca2099b273c1ae1c909a964b053077ead5c186c8b160c9d00",
    ...
    def test_get_height(self):
        ...
        self.assertEqual(
            resp["hash"],
            "228c0538b7ba7d28fdd58ed310326db61ea052038bdb42652f6e1852cf666325",
        )
    ...
    def test_get_transactions(self):
        ...
        resp = self.backend.get_transactions(
            [
                "c6802ca1805784c7330e71521a318c302f88625b0eab80740cfab24af7b0cb93",
                "ae476cae3ad2b44c6c2614b9435adbe304e24c59d410fe3b4dd046a9021d66b9",
            ],
            decode_as_json=True,
            prune=False,
        )
    ...
    def test_get_alt_blocks_hashes(self):
        ...
        hashes = [
            "9c2277c5470234be8b32382cdf8094a103aba4fcd5e875a6fc159dc2ec00e011",
            ...
            "697cf03c89a9b118f7bdf11b1b3a6a028d7b3617d2d0ed91322c5709acf75625",
            "d99b3cf3ac6f17157ac7526782a3c3b9537f89d07e069f9ce7821d74bd9cad0e",
            "e97b62109a6303233dcd697fa8545c9fcbc0bf8ed2268fede57ddfc36d8c939c",
            "70ff822066a53ad64b04885c89bbe5ce3e537cdc1f7fa0dc55317986f01d1788",
    ...
    def test_is_key_image_spent(self):
        ...
        key_imgs = [
            ...
            "8d1bd8181bf7d857bdb281e0153d84cd55a3fcaa57c3e570f4a49f935850b5e2",
            "fbbd6ac46dc4905c455f3b51595da6b135a5e9a64c6c181875558649be0ab183",
        ]
    ...
    def test_start_mining(self):
        ...
        with self.assertRaises(ValueError):
            self.backend.start_mining(
                False, True, "meepmoopthisisntarealminingaddress", 2
            )
    ...
    def test_mining_status_on(self):
        ...
        self.assertEqual(
            resp["address"],
            "497e4umLC7pfJ5TSSuU1QY8E1Nh5h5cWfYnvvpTrYFKiQriWfVYeVn2KH8Hpp3AeDRbCSxTvZuUZ1WYd8PGLqM4r5P5hjNQ",
        )
    ...
    def test_set_log_level(self):
        ...
        self.assertEqual(
            JSONRPCDaemon.known_log_levels(), JSONRPCDaemon._KNOWN_LOG_LEVELS
        )
    ...
    def test_set_log_categories_default(self):
        ...
        self.assertEqual(
            JSONRPCDaemon.known_log_categories(), JSONRPCDaemon._KNOWN_LOG_CATEGORIES
        )
    ...
    def test_get_transaction_pool(self):
        ...
        self.assertEqual(
            resp["spent_key_images"][0]["id_hash"],
            "1ccff101d8ee93903bde0a8ef99ebc99ccd5150f7157b93b758cd456458d4166",
        )
        self.assertEqual(
            resp["spent_key_images"][0]["txs_hashes"][0],
            "73fe0207e25d59dce5cd6f7369edf33a04ac56409eca3b28ad837c43640ef83f",
        )
        self.assertEqual(
            resp["transactions"][0]["id_hash"],
            "ea020cf2595c017d5fd4d0d427b8ff02b1857e996136b041c0f7fd6dffc4c72c",
        )
    ...
    def test_get_transaction_pool_stats(self):
        responses.add(
            responses.POST,
            self.gettransactionpoolstats_url,
    ...
    def test_get_outs(self):
        ...
        self.assertEqual(
            resp["outs"][0]["key"],
            "fc13952b8b9c193d4c875e750e88a0da8a7d348f95c019cfde93762d68298dd7",
        )
        self.assertEqual(
            resp["outs"][0]["mask"],
            "bf99dc047048605f6e0aeebc937477ae6e9e3143e1be1b48af225b41f809e44e",
        )
        self.assertEqual(
            resp["outs"][0]["txid"],
            "687f9b1d6fa409a13e84c682e90127b1953e10efe679c114a01d7db77f474d50",
        )
        ...
        self.assertEqual(
            resp["outs"][3]["key"],
            "e20315663e3d278421797c4098c828cad5220849d08c3d26fee72003d4cda698",
        )
        self.assertEqual(
            resp["outs"][3]["mask"],
            "100c6f1342b71b73edddc5492be923182f00a683488ec3a2a1c7a949cbe57768",
        )
        self.assertEqual(
            resp["outs"][3]["txid"],
            "2a5d456439f7ae27b5d26e493651c0e24e1d7e02b6d9d019c89d562ce0658472",
        )
    ...
    def test_get_outs_single(self):
        ...
        self.assertEqual(
            resp["outs"][0]["key"],
            "9c7055cb5b790f1eebf10b7b8fbe01241eb736b5766d15554da7099bbcdc4b44",
        )
        self.assertEqual(
            resp["outs"][0]["mask"],
            "42e37af85cddaeccbea6fe597037c9377045a682e66661260868877b9440af70",
        )
        self.assertEqual(
            resp["outs"][0]["txid"],
            "b357374ad4636f17520b6c2fdcf0fb5e6a1185fed2aef509b19b5100d04ae552",
        )
        self.assertEqual(resp["outs"][0]["unlocked"], True)
